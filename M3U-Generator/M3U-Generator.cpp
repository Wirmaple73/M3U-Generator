...
int wmain(int argc, wchar_t* argv[])
{
    ...
    const std::u8string newLine = args.DoesArgumentExist(CommandLineArgs::LINE_FEED_ONLY) ? u8"\n" : u8"\r\n";

    // Writing the BOM makes VLC cry over the first entry
    /*
    if (!args.DoesArgumentExist(CommandLineArgs::NO_BOM))
        outputFile.write("\xEF\xBB\xBF", 3);
    */

    if (!args.DoesArgumentExist(CommandLineArgs::NO_BRAG) && Utils::IsFileEmpty(outputPath))
        Utils::WriteUtf8StringToFilestream(outputFile, u8"# Generated by M3U Generator 1.0.0 (https://github.com/Wirmaple73/M3U-Generator)" + newLine);

    const MediaFormatChecker checker = InitMediaFormatChecker();
    ...
    for (int i = args.GetFirstRegularArgumentIndex(); i < argc; i++)
    {
        ...
        {
            Utils::WriteUtf8StringToFilestream(outputFile, path.u8string() + newLine);
            continue;
        }
        ...
        if (fs::is_directory(path, ec))
        {
            for (const fs::directory_entry& entry : fs::recursive_directory_iterator(path, fs::directory_options::skip_permission_denied, ec))
            {
                ...
                {
                    Utils::WriteUtf8StringToFilestream(outputFile, entry.path().u8string() + newLine);
                    continue;
                }
